package ui 
{
	...
	public class SmithMagicUi 
	{
		...
		public function main( skillId: Object) : void {
			...
			registerSlot(slot_item);
			registerSlot(slot_rune);
			registerSlot(slot_signature);
			//On vide la Grid
			updateItem(null);
		...
		public function onMouseCtrlDoubleClick(target:Object):void {
			switch (target)
			{
				case slot_rune:
					...
					if (target.data) sysApi.sendAction(new ExchangeObjectMove(target.data.objectUID, -(target.data.quantity)));
					break;
				default:
		...
		public function onDoubleClick(target : Object) : void {
			...
			switch (target)
			{
				...
				case slot_rune:
				case slot_signature:
					if (target.data) sysApi.sendAction(new ExchangeObjectMove(target.data.objectUID, -1));
				break;
				default:
					if (_runeRef[target] != null) {
						var data : ItemWrapper = _runeRef[target];
		...
		public function onRollOver(target : Object) : void 	{
			switch (target)
			{
				...
				case slot_signature:
				case slot_item:
					if (target.data) {uiApi.showTooltip(target.data.name, target);}
					break;
				default:
		...
		public function updateGrid(data : * , componentsRef : * , selected : Boolean) : void {
			...
			if (data !== null) {
				...
				for each (var effectDice : EffectInstanceDice in slot_item.data.possibleEffects) {
					if (effectDice.effectId == effect.effectId) {
						...
						if (signeBonus == 1) {
							...
							if (effectDice.diceSide == 0) { jetMax = jetMin; }
						}else {
							jetMin = signeBonus * effectDice.diceSide;
							jetMax = signeBonus * effectDice.diceNum;
							if (effectDice.diceSide == 0) { jetMin = jetMax; }
						}
				...
				if (isExotic) { componentsRef.lb_jet.cssClass = "exotic"; 
					componentsRef.tx_bulle.uri = uiApi.createUri((uiApi.me().getConstant("assets") + "state_7")); }

				//On enregistre les slots rune simple/pa/ra
				registerSlot(componentsRef.slot_simple);
				registerSlot(componentsRef.slot_pa);
				registerSlot(componentsRef.slot_ra);
		...
		private function updateRune(item : ItemWrapper) : void {	
			...
			if (item == null) { lbl_rune_name.text = ""; lbl_rune_effect.text = ""; lbl_rune_poids.text = ""; return; }
		...
		private function updateItem(item : ItemWrapper) : void {
			...
			if (item == null) { maGrid.dataProvider = tabEffect; return; }
